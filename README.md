...
- Using `.map()`, create an `<option>` element for each item of the `currencyData` array. Each `<option>` element should have a `key`, an `id`, and have the individual currency name as text. Call the new array `currencyOptions`.
- Inside of the `return` of the `render()` method, create a container `<div>`. Inside of the `<div>` create a `<select>` element with an attribute of `value={this.state.selectedCurrency}`.
- Inside of the `<select>` create a single `<option>` element with an attribute of `value='Select Currency'` and 'Select Currency' as the inner text. Below that, inside of `{}` display the `currencyOptions`.
...
- Let's start off by creating a method that will handle the increase of the amount on state. We will be using the public class field syntax and using `setState` with a callback, rather than an object. The callback will take one parameter, `prevState`. This parameter gives us access to state without modifying it directly.
